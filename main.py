...
# Internal Gateway Configs (Never Exposed to Users)
...
# Root Index: Clean Documentation (No Keys, No Source Information)
@app.get("/")
def gateway_index():
    return {
        "status": True,
        ...
        "endpoints": [
            ...
            },
            {
                ...
                "example_usage": "/api/v1/lookup?mobile=9876543210&api_key=[YOUR_KEY]"
            }
        ]
    }
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if extracted:
                ...
                for item in dataset:
                    parsed.append({
                        "mobile": item.get("mobile", target),
                        "name": item.get("name", "N/A"),
                        "father_name": item.get("fname") or item.get("father_name", "N/A"),
                        "address": item.get("address", "N/A"),
                        "circle": item.get("circle", "N/A"),
                        "email": item.get("email", "N/A"),
                        "alt_mobile": item.get("alt") or item.get("alt_mobile", "N/A"),
